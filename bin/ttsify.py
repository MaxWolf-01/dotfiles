...
def main(input_path: Path, output: Path | None, model: str, debug: bool):
    ...
    default_rules = (
        ...
        "Your task: TRANSFORM and PRESERVE the COMPLETE paper content into clear narration.\n"
        "\n"
        "CRITICAL: You MUST transform the ENTIRE input. Do NOT truncate, summarize, or write meta-comments like 'content continues...' or 'I've omitted...'. Process EVERY paragraph.\n"
        "\n"
        "Rules:\n"
        "- KEEP ALL substantive content: abstract, introduction, methods, results, analysis, discussion, conclusion, etc.\n"
        "- Preserve the paper's structure (title, sections) and rewrite content for smooth narration.\n"
        "- Replace formulas with spoken descriptions (e.g., 'x squared plus y').\n"
        "- Omit complex tables and equations (but mention their presence: 'Table 1 shows...', 'Equation 2 defines...').\n"
        "- Keep figure captions, but remove markdown image links like ![img-0.jpeg](file.jpeg).\n"
        "- Remove inline citation markers like '[1][2]' or '(Smith et al., 2020)', but keep author names in running text; turn 'et al.' into 'and colleagues'.\n"
        "- ONLY omit sections that are purely bibliographies (titled 'References', 'Bibliography', 'Works Cited', etc.) containing nothing but citation lists.\n"
        "- Omit metadata (DOI, arXiv ID, publication venue, author emails/affiliations). Omit repeated headers/footers.\n"

...
Existing scripts include `tre` (gitignore-aware tree), `extract` (universal archive extractor), `clip` (clipboard, X11/Wayland-aware), `fext` (disk usage by extension), `restic-*` helpers (restic-diff, restic-snaps, restic-lsfs, restic-ls-recent, restic-ls-all, restic-current), `backup-staleness-check` (alarms on repos that stopped receiving snapshots), `claude-redact-secrets` (strips API keys and tokens out of old Claude Code transcripts; dry run unless `--apply`), and `vault-*` (knowledge base publishing tools — see below). If a common operation is needed repeatedly, add a new script here.
